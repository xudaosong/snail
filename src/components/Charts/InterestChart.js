...
export default class InterestChart extends PureComponent {
  static propTypes = {
  ...
  renderChart(portfolio) {
    ...
    let interestTotal = _.sumBy(portfolio, (o) => (new Decimal(o.interest).plus(o.platformRewardInterest).plus(o.channelRewardInterest).toNumber()))
    let data = _.map(_.orderBy(portfolio, ['principal'], ['desc']), (item) => {
      let interest = new Decimal(item.interest).plus(item.platformRewardInterest).plus(item.channelRewardInterest)
      let percent = _.round(interest.div(interestTotal).mul(100).toNumber(), 2)
      return {
        name: `${item.platform} ${percent}%`,
        value: interest.toNumber()
      }
    })
    let option = {
      title: {
        text: formatCurrency(interestTotal)
      },
      series: [{
        data
      }]
